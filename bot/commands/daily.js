...
/**
 * Pure report generator for Daily Active Users (shared between command and refresh callback).
 */
export async function generateDailyReport(env, { projectArg = "", days = 7 } = {}) {
    ...
    if (successfulCount > 0) {
        ...
        reportId = await saveReportContext(env, 'daily', chartResults, {
            isFiltered,
            projectName: matchedName,
            days,
            queryParams: { type: 'daily', projectArg, days }
        });
        keyboard.text("📈 View as Chart", `chart:${reportId}`);
    }

    const refreshCb = buildRefreshCallback('daily', { projectArg, days }, reportId);
...
export function setupDailyCommand(bot) {
    bot.command("daily", async (ctx) => {
        ...
        try {
            const report = await generateDailyReport(ctx.env, { projectArg, days });
